...
import { 
    Plus, PlayCircle, FileText, Camera, Scissors, CheckCircle, Maximize2 
} from 'lucide-react';
...
export default function WorkstationPage() {
    ...
    const activeYoutube = contents.filter(c => 
        c.platform === 'youtube' && 
    ...
    return (
        <div className={cn(
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight">Production Workstation</h1>
                    <p className="text-muted-foreground">Manage your active video pipeline.</p>
                </div>
                <div className="flex items-center gap-4">
                    ...
                    <Button onClick={handleCreate} size="lg" className="bg-red-600 hover:bg-red-700 shadow-lg shadow-red-600/20">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {activeYoutube.map((project) => (
                            <Card 
                                ...
                                onClick={() => router.push(`/dashboard/content/youtube/project/${project._id}`)}
                            >
                                <CardContent className="p-6 space-y-4">
                                    <div className="flex justify-between items-start">
                                        <Badge variant="outline" className={cn(
                                            "uppercase tracking-wider text-[10px] font-bold",
                                            project.status === 'idea' ? "bg-yellow-500/10 text-yellow-600 border-yellow-200" :
                                            project.status === 'scripting' ? "bg-orange-500/10 text-orange-600 border-orange-200" :
                                            project.status === 'filming' ? "bg-red-500/10 text-red-600 border-red-200" :
                                    ...
                                    <div className="pt-2 flex items-center gap-4 text-xs text-muted-foreground">
                                        <div className="flex items-center gap-1">
                                            ...
                                            {project.tasks?.filter(t => t.isCompleted).length || 0} tasks done
                                        </div>
                                    </div>
