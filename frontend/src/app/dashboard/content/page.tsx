...
export default function ContentOverviewPage() {
  ...
  // ... stats calculation stays same ...
  const stats = {
    total: contents.length,
    published: contents.filter((c) => c.status === "published").length,
    inProgress: contents.filter((c) =>
  ...
  return (
    <div className="min-h-full space-y-8 animate-in fade-in duration-500 pb-10">
      ...
      {/* HERO HEADER */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 bg-linear-to-br from-white to-zinc-50 dark:from-zinc-900/50 dark:to-zinc-950 p-8 rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-sm relative overflow-hidden group">
        ...
        <div className="relative z-10 space-y-2">
          ...
          <h1 className="text-4xl font-black tracking-tighter text-foreground">
            Content OS{" "}
            <span className="text-xs align-top bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded-full text-muted-foreground ml-1">
              v2.0
            </span>
          </h1>
        ...
        <div className="flex gap-3 relative z-10">
          <Button
            variant="outline"
            className="h-12 px-6 border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 shadow-sm hover:bg-zinc-50 dark:hover:bg-zinc-800"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <Card
            key={i}
            className="border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {PLATFORM_CARDS.map((platform) => (
            <Link
              ...
              className={cn(
                "block group h-full",
                platform.href === "#" && "pointer-events-none opacity-60"
              )}
            >
              <div
                className={cn(
                  "h-full p-1 rounded-3xl bg-linear-to-br transition-all duration-300 shadow-sm hover:shadow-xl group-hover:-translate-y-1 relative overflow-hidden",
                  platform.gradient,
                  ...
                  <div className="relative z-10 flex flex-col h-full justify-between">
                    <div>
                      <div className="flex justify-between items-start mb-6">
                        ...
                        <Badge
                          variant="outline"
                          className="bg-background/80 backdrop-blur border-zinc-200 dark:border-zinc-700"
                        >
        ...
        <div className="space-y-4">
          {contents.length > 0 ? (
            contents.slice(0, 3).map((item) => {
              const PlatformIcon =
                PLATFORM_CARDS.find((p) => p.id === item.platform)?.icon ||
                FileText;
              const platformColor =
                PLATFORM_CARDS.find((p) => p.id === item.platform)?.color ||
                "text-zinc-500";
              return (
                <div
                  key={item._id}
                  className="flex items-center justify-between p-4 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-100 dark:border-zinc-800 shadow-sm hover:shadow-md transition-shadow"
                >
