...
export default function TwitterStudioPage() {
  ...
  const [threadTweets, setThreadTweets] = useState([""]); // Start with one empty tweet
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-[#0f0f0f] text-foreground font-sans flex flex-col overflow-hidden">
      <div className="bg-background pt-4 px-6 border-b border-zinc-200 dark:border-zinc-800">
        <ContentNav />
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col h-full overflow-hidden border-r border-zinc-200 dark:border-zinc-800">
          {/* HEADER */}
          <header className="h-16 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#0a0a0a] flex items-center justify-between px-6 shrink-0">
          ...
          <div className="flex-1 overflow-y-auto bg-zinc-50 dark:bg-[#0f0f0f] relative">
            ...
            <div className="max-w-2xl mx-auto py-10 px-6 space-y-6">
              {/* THREAD EDITOR VIEW */}
              <AnimatePresence>
                {(threadMode ? threadTweets : [tweetContent]).map(
                  (tweet, idx) => (
                    <motion.div
                      key={idx}
                    ...
                    >
                      ...
                      {threadMode && idx < threadTweets.length - 1 && (
                        <div className="absolute left-9 top-16 bottom-[-24px] w-0.5 bg-zinc-200 dark:bg-zinc-800 z-0" />
                      )}

                      <div className="flex gap-4">
                        <div className="shrink-0 pt-2 relative z-10">
                          <div className="w-10 h-10 rounded-full bg-linear-to-br from-indigo-500 to-purple-500 shadow-lg" />
                        </div>
                        ...
                        <div className="flex-1 space-y-3">
                          <div className="w-full bg-white dark:bg-[#1a1a1a] rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm focus-within:ring-2 ring-sky-500/20 transition-all p-4">
                            <div className="flex justify-between items-start mb-2">
                              ...
                              {threadMode && idx > 0 && (
                                <Button
                                  ...
                                  onClick={() => {
                                    const newTweets = threadTweets.filter(
                                      (_, i) => i !== idx
                                    );
                                    setThreadTweets(newTweets);
                                  }}
                            ...
                            <Textarea
                              ...
                              onChange={(e) =>
                                threadMode
                                  ? handleUpdateThreadTweet(idx, e.target.value)
                            ...
                            <div className="pt-4 mt-2 border-t border-zinc-100 dark:border-zinc-800/50 flex items-center justify-between">
                              <div className="flex items-center gap-1 text-sky-500">
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  className="hover:bg-sky-500/10 rounded-full h-8 w-8"
                                >
          ...
          <div className="h-16 border-t border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#0a0a0a] px-6 flex items-center gap-4 overflow-x-auto">
            ...
            {THREAD_TEMPLATES.map((tmpl, i) => (
              <Button
                ...
                variant="outline"
                size="sm"
                className="gap-2 rounded-full border-zinc-200 dark:border-zinc-800 bg-transparent hover:bg-zinc-50 dark:hover:bg-zinc-900"
              >
                <tmpl.icon className={cn("w-3 h-3", tmpl.bg.split(" ")[1])} />{" "}
                {tmpl.label}
              </Button>
            ))}
          </div>
        ...
        <div className="w-[400px] flex flex-col bg-white dark:bg-[#0a0a0a] border-l border-zinc-200 dark:border-zinc-800">
          <Tabs defaultValue="preview" className="flex-1 flex flex-col">
            <TabsList className="grid grid-cols-2 rounded-none p-0 h-14 bg-transparent border-b border-zinc-200 dark:border-zinc-800">
              <TabsTrigger
                value="preview"
                className="rounded-none border-b-2 border-transparent data-[state=active]:border-sky-500 data-[state=active]:text-sky-500 h-full"
              >
                Mobile Preview
              </TabsTrigger>
              <TabsTrigger
            ...
            <TabsContent
              value="preview"
              className="flex-1 bg-zinc-100 dark:bg-zinc-900 p-6 flex items-center justify-center overflow-auto m-0"
            >
              {/* PHONE MOCKUP */}
                    ...
                    (txt, i) => (
                      ...
                      >
                        ...
                        <div className="flex gap-3">
                          <div className="w-10 h-10 rounded-full bg-linear-to-br from-indigo-500 to-purple-500 shrink-0 z-10 relative" />
            ...
            >
              <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
                {[1, 2, 3, 4, 5].map((_, i) => (
                  <div
                    key={i}
                    className="p-4 hover:bg-zinc-50 dark:hover:bg-zinc-900/50 cursor-pointer"
                  >
                    <div className="flex justify-between items-start mb-2">
                      <Badge
                        variant="outline"
                        className="text-[10px] bg-sky-500/5 text-sky-500 border-sky-500/20"
                      >
                        Scheduled
                      </Badge>
                    ...
                    <p className="text-sm line-clamp-2 text-foreground/80">
                      Just shipped a new update to the dashboard. The new
                      performance metrics are insane! 🚀 #buildinginpublic
                    </p>
                  </div>
                ))}
              </div>
