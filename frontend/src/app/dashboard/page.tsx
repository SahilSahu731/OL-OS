...
const QUOTES = [
  ...
  {
    text: "If you can't do something, then don't. Believe in someone who can.",
  ...
  {
    text: "No matter how deep the night, it always turns to day.",
  ...
  {
    text: "If you can't do it, then don't. Believe in someone who can.",
...
export default function DashboardPage() {
  ...
  const { tasks, logs, fetchTasks, fetchLogs, toggleLog, metrics } =
    useTaskStore();
  ...
  const progressPercent = Math.min(
    100,
    Math.max(0, ((xp - currentLevelBaseXp) / 1000) * 100)
  );

  const getRank = (lvl: number) => {
    if (lvl >= 100)
      return { title: "Grandmaster", icon: Crown, color: "text-yellow-500" };
    if (lvl >= 50)
      return { title: "Elite", icon: Trophy, color: "text-purple-500" };
    if (lvl >= 25)
      return { title: "Veteran", icon: Zap, color: "text-red-500" };
    if (lvl >= 10)
      return { title: "Apprentice", icon: Target, color: "text-blue-500" };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-zinc-950 text-white shadow-2xl border border-zinc-800">
        ...
        <div className="absolute inset-0 bg-linear-to-t from-zinc-950 via-transparent to-zinc-900/50"></div>
        ...
        <div className="relative z-10 p-8 pt-4 md:p-10 flex flex-col md:flex-row items-center md:items-end justify-between gap-8">
          <div className="space-y-4 text-center md:text-left">
            <div className="flex items-center gap-3 justify-center md:justify-start">
              <span
                className={cn(
                  "px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest bg-white/10 backdrop-blur-md border border-white/20",
                  rank.color
                )}
              >
                {rank.title} Rank
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-4 gap-8">
        {/* ... stats cards ... */}
        <Card className="md:col-span-1 bg-linear-to-br from-orange-500/10 to-transparent border-orange-500/20 shadow-lg hover:shadow-orange-500/10 transition-shadow">
        ...
        <Card className="md:col-span-1 bg-linear-to-br from-emerald-500/10 to-transparent border-emerald-500/20 shadow-lg hover:shadow-emerald-500/10 transition-shadow">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        {/* FINANCE CARD */}
        <Card
          className="md:col-span-2 bg-linear-to-br from-yellow-500/5 to-transparent border-zinc-200 dark:border-zinc-800 shadow-lg relative overflow-hidden hover:border-yellow-500/20 transition-colors cursor-pointer"
        ...
        >
          ...
          <CardContent className="space-y-1 relative z-10">
            <div className="text-4xl font-bold tracking-tight">
              {formatCurrencyValue(summary.balance, currency)}
            </div>
            <div className="flex gap-4 text-sm font-medium pt-2">
              <span className="text-emerald-500 flex items-center gap-1">
                <TrendingUp className="w-3 h-3" /> +
                {formatCurrencyValue(summary.totalIncome, currency)}
              </span>
              <span className="text-red-500">
                -{formatCurrencyValue(summary.totalExpense, currency)}
              </span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card
          className="md:col-span-2 border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 cursor-pointer hover:border-blue-500/20 transition-colors group relative overflow-hidden"
              ...
              <div className="space-y-3">
                <div className="p-3 bg-white dark:bg-zinc-950 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
                  <div className="text-[10px] text-muted-foreground uppercase font-bold mb-1 flex items-center gap-1">
                    <Activity className="w-3 h-3 text-emerald-500" /> Last
                    Session
                  </div>
                  {workouts.length > 0 ? (
                    <div>
                      ...
                      <div className="text-[10px] text-muted-foreground font-mono mt-1">
                        {format(parseISO(workouts[0].date), "MMM dd")} •{" "}
                ...
                <div className="p-3 bg-white dark:bg-zinc-950 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex items-center justify-between">
                  <div>
                    ...
                    <div className="flex items-end gap-1">
                      ...
                      <span className="text-[10px] text-muted-foreground mb-1 font-bold">
                        lbs
                      </span>
                    </div>
                  </div>
                ...
                return (
                  <div className="p-4 bg-zinc-900 rounded-xl border border-zinc-800 relative overflow-hidden flex flex-col justify-between">
                    ...
                    <div className="relative z-10 my-2">
                      ...
                      <Progress
                        value={Math.min(100, (cal / calTarget) * 100)}
                        className="h-1.5 bg-zinc-800 mt-2"
                        indicatorClassName="bg-linear-to-r from-orange-600 to-yellow-500"
                      />
                    </div>
                      ...
                      <div className="flex flex-col items-center flex-1">
                        <div className="h-1 w-full bg-red-500/20 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, (p / pTarget) * 100)}%`,
                            }}
                          />
                        </div>
                      ...
                      <div className="flex flex-col items-center flex-1 mx-2">
                        <div className="h-1 w-full bg-blue-500/20 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, (c / cTarget) * 100)}%`,
                            }}
                          />
                        </div>
                        <span className="text-[9px] font-bold text-zinc-400 mt-1">
                      ...
                      <div className="flex flex-col items-center flex-1">
                        <div className="h-1 w-full bg-yellow-500/20 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, (f / fTarget) * 100)}%`,
                            }}
                          />
                        </div>
                        <span className="text-[9px] font-bold text-zinc-400 mt-1">
        ...
        <div className="md:col-span-4 grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Quick Capture (Note) */}
          <Card
            className="group relative overflow-hidden border-zinc-200 dark:border-zinc-800 cursor-pointer"
        ...
        <div className="md:col-span-4 grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3 space-y-8">
            ...
            <Card className="bg-linear-to-r from-red-500/10 to-transparent border-red-500/20">
              <CardHeader className="pb-2">
              ...
              <CardContent className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Day 12 of 30 • Phase 1: Foundation
                  </p>
                </div>
                <Button
                  size="sm"
                  ...
                  onClick={() => router.push("/dashboard/habits/challenges")}
                >
                  View Intel
                </Button>
              </CardContent>
            ...
            <Card className="flex flex-col border-zinc-200 dark:border-zinc-800 shadow-xl overflow-hidden min-h-[400px]">
              <CardHeader className="bg-zinc-50/50 dark:bg-zinc-900/50 border-b border-zinc-100 dark:border-zinc-800">
                <CardTitle className="flex items-center gap-2">
                  <LayoutList className="w-5 h-5 text-primary" /> Today&apos;s
                  Mission
                </CardTitle>
                <CardDescription>
              ...
              <CardContent className="flex-1 overflow-y-auto max-h-[400px] space-y-2 p-4 custom-scrollbar">
                {tasks.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-muted-foreground py-10">
                    ...
                    <Button
                      variant="link"
                      onClick={() => router.push("/dashboard/habits/track")}
                    >
                      Initialize Habit Protocols
                    </Button>
                  </div>
                ) : (
                  tasks.map((task) => {
                          ...
                          <div>
                            <p
                              className={cn(
                                ...
                                isDone &&
                                  "text-muted-foreground line-through decoration-zinc-500/50"
                              )}
                            >
                              {task.title}
                            </p>
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-[10px] uppercase font-bold text-muted-foreground bg-secondary px-2 py-0.5 rounded-md tracking-wider">
                                {(typeof task.category === "string"
                                  ? task.category
                                  : task.category?.name) || "General"}
                              </span>
                              {taskStreak > 0 && (
                                <span className="text-[10px] text-orange-500 bg-orange-500/10 px-2 py-0.5 rounded-md font-bold flex items-center gap-1">
                                  <Flame className="w-3 h-3" /> {taskStreak} Day
                                  Streak
                                </span>
                              )}
                            </div>
          ...
          <div className="lg:col-span-1 flex flex-col gap-6">
            ...
            <Card className="border-zinc-200 dark:border-zinc-800 shadow-lg relative overflow-hidden h-48 bg-zinc-950">
              <div
                className={cn(
                  "absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-20 mix-blend-overlay transition-opacity duration-1000",
                  isPrime ? "opacity-40" : "opacity-20"
                )}
              />
              <div
                className={cn(
                  "absolute inset-0 bg-linear-to-br from-indigo-500/20 via-purple-500/10 to-transparent transition-opacity duration-1000",
                  isPrime ? "opacity-100" : "opacity-0"
                )}
              />
              ...
              <CardContent className="relative z-10 h-full flex flex-col justify-between p-6">
                <div className="flex justify-between items-start">
                  ...
                  {isPrime && (
                    <div className="flex gap-1">
                      <span className="w-1 h-3 bg-indigo-500 animate-pulse delay-75" />
                      <span className="w-1 h-4 bg-indigo-500 animate-pulse delay-150" />
                      <span className="w-1 h-3 bg-indigo-500 animate-pulse delay-0" />
                    </div>
                  )}
                </div>
                ...
                <div>
                  <h3 className="text-xl font-bold text-white mb-2">
                    {isPrime ? "Neural Sync Active" : "Silence"}
                  </h3>
                  <Button
                    size="sm"
                    className={cn(
                      ...
                      isPrime
                        ? "bg-indigo-500 hover:bg-indigo-400 text-white shadow-[0_0_20px_rgba(99,102,241,0.5)]"
            ...
            <Card className="bg-zinc-950 text-white border-zinc-800 shadow-2xl relative overflow-hidden group hover:border-zinc-700 transition-colors">
              ...
              <CardHeader className="pb-2 relative z-10">
                <CardTitle className="text-xs uppercase tracking-widest text-zinc-500 flex items-center gap-2">
                  <Quote className="w-3 h-3" /> Daily Wisdom
                </CardTitle>
              </CardHeader>
              <CardContent className="relative z-10">
                <blockquote className="space-y-4">
                  <p className="text-xl font-serif italic leading-relaxed text-zinc-100">
                    &ldquo;{quote.text}&rdquo;
                  </p>
                  <footer className="text-sm text-primary font-bold tracking-wide">
                    — {quote.author}
                  </footer>
                </blockquote>
              </CardContent>
            ...
            <Card className="border-zinc-200 dark:border-zinc-800 shadow-lg flex flex-col">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
                  <Zap className="w-3 h-3" /> Neural Link
                </CardTitle>
              </CardHeader>
            ...
            <Card className="border-zinc-200 dark:border-zinc-800 shadow-lg">
              ...
              <CardContent className="grid grid-cols-2 gap-3">
                <Button
                  variant="outline"
                  className="h-20 flex flex-col gap-2 hover:border-primary/50 hover:bg-primary/5 space-y-1"
                  onClick={() => router.push("/dashboard/habits/track")}
                >
                ...
                <Button
                  variant="outline"
                  className="h-20 flex flex-col gap-2 hover:border-green-500/50 hover:bg-green-500/5 space-y-1"
                  onClick={() => router.push("/dashboard/finance")}
                >
                ...
                <Button
                  variant="outline"
                  className="h-20 flex flex-col gap-2 hover:border-blue-500/50 hover:bg-blue-500/5 space-y-1"
                  onClick={() => router.push("/dashboard/vault")}
                >
                ...
                <Button
                  variant="outline"
                  className="h-20 flex flex-col gap-2 hover:border-indigo-500/50 hover:bg-indigo-500/5 space-y-1"
                  onClick={() => router.push("/dashboard/focus")}
                >
            ...
            <Card className="border-zinc-200 dark:border-zinc-800 shadow-sm relative overflow-hidden">
              ...
              <CardContent className="grid grid-cols-4 gap-2">
                {[1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className="aspect-square rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center border border-dashed border-zinc-300 dark:border-zinc-700 hover:border-primary hover:text-primary transition-colors cursor-pointer"
                  >
