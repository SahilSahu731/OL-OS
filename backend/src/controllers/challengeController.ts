...
export const getChallenges = async (req: Request, res: Response) => {
    try {
        ...
        const challenges = await Challenge.find({ userId }).sort({ status: 1, createdAt: -1 });
...
export const logDailyProgress = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
...
export const failChallenge = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        const challenge = await Challenge.findById(id);
